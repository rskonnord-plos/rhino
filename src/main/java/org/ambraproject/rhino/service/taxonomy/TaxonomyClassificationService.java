...
public interface TaxonomyClassificationService {
  ...
  /**
   * Queries the MAI server for taxonomic terms for a given article, and returns a list of the raw results. If {@param
   * isTextRequired} is true, it will also return the text that is sent to taxonomy server.
   *
   ...
   * that we don't currently store in mysql. The first element of the list will be the text sent to the server if
   * {@param isTextRequired} is true.
   * @throws IOException
   */
